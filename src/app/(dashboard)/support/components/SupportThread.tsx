  ...
  const fetchReplies = async () => {
    try {
      ...
      const response: any = await apiGet(
        `/api/support/getReplies?ticketId=${ticketId}`
      );
      console.log("API Response:", response); // Debug log
      if (response.success) {
        setReplies(response.data);
        setTicket(response.ticket);
        console.log("Ticket data:", response.ticket); // Debug log
        console.log("Subject:", response.ticket?.subject); // Debug subject
        console.log("Message:", response.ticket?.message); // Debug message
        console.log("All ticket fields:", Object.keys(response.ticket || {})); // Debug all fields
      } else {
        setError(response.message);
      }
    } catch (err) {
      console.error("Error fetching replies:", err); // Debug log
  ...
  const markAllRepliesAsRead = async () => {
    try {
      ...
      const unreadUserReplies = replies.filter(
        (reply) => reply.senderType === "user" && !reply.isRead
      );
