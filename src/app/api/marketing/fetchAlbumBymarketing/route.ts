...
export async function GET() {
  try {
    ...
    // Fetch all marketing entries with timeout handling
    const marketingEntries = await Marketing.find().sort({ _id: -1 }).lean().maxTimeMS(10000);
    ...
    // Process entries in batches to avoid overwhelming the connection pool
    const batchSize = 10;
    const marketingWithAlbumDetails = [];

    for (let i = 0; i < marketingEntries.length; i += batchSize) {
      const batch = marketingEntries.slice(i, i + batchSize);
      
      const batchResults = await Promise.all(
        batch.map(async (marketing) => {
          try {
            // Fetch album details based on the albumId in the marketing entry
            const album = await Album.findById(marketing.albumId).maxTimeMS(5000);
      ...
      marketingWithAlbumDetails.push(...batchResults);
    }
